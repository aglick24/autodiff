...
class Result:
    ...
    def clone(self, remove_level=None):
        ...
        """
        resclone = Result(self.val, self.op, self.parents,
                          self.trace_levels.copy())
...
def _get_shape(arg):
    assert not isinstance(
        arg, list), "lists have no shape; arg should be numeric or an ndarray!"
...
class MyBroadcast(MyOp):
    ...
    def compute_op(self, arg0, shape, axis):
        ...
        assert isinstance(arg0, float) or len(
            arg0.shape) == 1, "can only broadcast floats or vectors"
        # scalar case
        ...
        return np.tile(arg0.reshape(-1, 1), (1, shape[1]))
...
square, tanh, cosh, sinh, exp, log = MySquare(
), MyTanh(), MyCosh(), MySinh(), MyExp(), MyLog()
...
def put_vjp_for_arg(argidx, gradout, result, args):
    if argidx == 0:
        alist = args[0].get_val() if isinstance(args[0], Result) else args[0]
        nulist = [np.zeros(thing.shape) if i == args[1] else get(gradout, i)
...
def tanh_vjp_for_arg(argidx, gradout, result, args):
    return div(gradout, square(cosh(args[0])))
...
def dimmax_vjp_for_arg(argidx, gradout, result, args):
    ...
    if argaxis is None:  # we summed to a scalar
        ...
        return mult(mask, bdcast(gradout, argshape, None))
    assert len(gshape) == 1
...
def bdcast_vjp_for_arg(argidx, gradout, result, args):
    ...
    arg0 = args[0]
    assert isinstance(arg0, float) or len(
        arg0.shape) == 1, "could've only broadcast floats or vectors"
